...
async function getRecipes() {
  ...
  if(localStorage.getItem('recipe') != null && localStorage.getItem('recipe') != '[]' ){
    rec = localStorage.getItem('recipe');
  ...
  //            you can call to either resolve the Promise or Reject it.
    return new Promise(async(resolve, reject) =>{
      for(let i =0; i< RECIPE_URLS.length; i++){
        let urls = RECIPE_URLS[i]
        try{
          let getThigns = await fetch(urls)
          getThigns = await getThigns.json()
          recipeFetch.push(getThigns)
        }
        catch(error){
          console.error(error);
          rejectError.push(error);
        }
      }
